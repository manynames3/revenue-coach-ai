...
- Call intake stores `consent_confirmed` and optional `consent_notes` so managers can record the basis for uploading a conversation.
- `/account` exposes workspace-level trust controls for retention days, consent requirements, and redaction policy flags.
...
- Background enforcement for the configured retention policy for transcripts, audio, and raw AI JSON.
